...
*/
WITH CTE AS(
    SELECT  p.product_id AS product_id , s.year AS first_year , s.quantity AS quantity, s.price as price,
    RANK() OVER(PARTITION BY p.product_id ORDER BY s.year) AS RNK FROM Sales s 
    JOIN Product p ON
